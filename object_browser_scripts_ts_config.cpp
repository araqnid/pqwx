...
void TextSearchConfigurationScriptWork::GenerateMappings(OutputIterator output)
{
  ...
  wxString mappingSql;
  int lastTokenType = -1;
  for (QueryResults::rows_iterator iter = mapping.Rows().begin(); iter != mapping.Rows().end(); iter++) {
    const QueryResults::Row& row = *iter;
    int tokenType = row.ReadInt4(0);
    wxString dictName = QuoteIdent(row.ReadText(1));
    wxString dictSchemaName = QuoteIdent(row.ReadText(2));
